...
export const test_schema_separate_array = (): void => {
  ...
  const auth: IOpenAiSchema = schema(
    typia.json.application<[Authorization[]]>(),
  );
  const upload: IOpenAiSchema = schema(
    typia.json.application<[FileUpload[]]>(),
  );
  const combined: IOpenAiSchema = schema(
    typia.json.application<[Combined[]]>(),
  );
  ...
  TestValidator.equals("combined")(separator(combined))([
    member,
    schema(typia.json.application<[Array<Authorization & FileUpload>]>()),
  ]);
};
